...
*   **Implementation Agents:** These are independent AI entities tasked with executing discrete segments of the Implementation Plan. They perform the core development or content generation tasks and are responsible for meticulously logging their processes and outcomes to the Memory Bank.
...
1.  **Verification of APM Framework Asset Availability:**
    *   To ensure operational consistency, it is essential for you to understand how the APM framework is set up for this project. For this project, the APM framework has been reorganized from the standard GitHub repository structure. The current project structure is:

        ```
        project-root/
        ├── .github/                                        # APM framework assets reorganized here
        │   ├── context/                                    # Project-specific context files
        │   │   ├── hybrid-linters-summary.md              # Summary of hybrid linter concepts
        │   │   └── implementation-plan.md                 # Main Implementation Plan for the project
        │   ├── instructions/                              # IDE-specific instruction files
        │   │   ├── apm_discovery_synthesis_reminder.instructions.md
        │   │   ├── apm_impl_plan_critical_elements_reminder.instructions.md
        │   │   ├── apm_memory_naming_validation_reminder.instructions.md
        │   │   ├── apm_memory_system_format_source.instructions.md
        │   │   ├── apm_plan_format_source.instructions.md
        │   │   └── apm_task_prompt_plan_guidance_incorporation_reminder.instructions.md
        │   ├── memory/                                     # Memory Bank directory for agent logs
        │   │   ├── README.md                               # Memory Bank organization guide
        │   │   ├── Phase_1_Foundation/                     # Phase 1 task logs
        │   │   ├── Phase_2_Plugin_System/                  # Phase 2 task logs
        │   │   ├── Phase_3_Advanced_Features/              # Phase 3 task logs
        │   │   └── Phase_4_Developer_Experience/           # Phase 4 task logs
        ...
        │       │   ├── 01_Initiation.prompt.md             # (This file) Primary prompt to initiate the Manager Agent
        │       │   └── 02_Codebase_Guidance.prompt.md      # Prompt for MA to guide codebase/project discovery
        ...
        └── [project files...]                              # Main project source code and assets
        ```
        
        **Note:** This project uses a customized APM structure where:
        - All APM framework assets are located under `.github/` instead of the root
        - The Implementation Plan is stored at `.github/context/implementation-plan.md`
        - The Memory Bank system is located at `.github/memory/` with phase-based organization
        - Additional `context/` and `instructions/` directories provide project-specific guidance
        - Core prompts and guides remain in their standard relative locations under `.github/prompts/`
